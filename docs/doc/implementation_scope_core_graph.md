...
- 已新增 `tests/mvp_runtime.rs` crate-level 集成测试，从公开 API 视角覆盖 `StateGraph::with_channels`、`compile`、`invoke(Values)`、`invoke(Updates)` chunk 收集、默认 `stream`、`stream(Updates)`、`RuntimeContext.stream_mode` 覆盖、节点读取 `RuntimeContext.context`、条件边、waiting edge、多次运行隔离、空输入错误和 unsupported command。
