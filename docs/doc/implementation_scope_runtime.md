...
- `Pregel::stream(input)` 已使用 `tokio::sync::mpsc` 返回管道 receiver，并由后台 task 建立 `PregelLoop`，随后立即调用 `enter()`，贴近源项目 `with SyncPregelLoop(...) as loop:` 的调用顺序。后台主循环现在可按 `tick -> execute -> after_tick` 推进到 `Done` 或递归限制错误；`execute` 和 `after_tick` 不返回 stream chunk，values/updates 发送协议后续再补。`CompiledStateGraph` 通过 `Arc<Pregel>` 让后台 task 持有图规格，实际 loop 仍借用规格字段；每次运行复制 channels 和 managed，避免不同 stream 调用共享可变运行态。
- 已为 `RuntimeContext` 用户上下文字段和 `GraphError` 展示文本补充基础单元测试。

## 当前未完成

- `invoke` 尚未实现；`stream()` 当前只暴露管道和 loop 构造边界，尚未提供同步 Pregel 主循环的真实执行结果。
- `PregelLoop::tick` 和 `PregelLoop::after_tick` 已接入最小 Plan/Update 闭环，因此 `stream()` 后台主循环能端到端推进图执行；但当前仍不发送 values/updates stream item。
