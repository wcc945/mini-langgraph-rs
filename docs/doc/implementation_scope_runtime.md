...
- `src/runtime/mod.rs` 已开始定义 `RuntimeContext<ContextT>`，当前包含用户运行上下文 `context: ContextT` 和可选 `stream_mode: Option<StreamMode>`。该上下文按源项目 `Runtime.context` 的定位作为运行期只读依赖视图使用，图状态修改必须通过节点输出和 channel writes 表达；`stream_mode` 用于覆盖本次运行的默认 stream 输出模式。
...
- `CompiledStateGraph` 已能由 `StateGraph::compile()` 生成，并持有可校验的 `Pregel` 容器；当前已提供 `invoke(input, runtime_context)` 和 `stream(input, runtime_context)` 转发到内部 `Pregel`，stream mode 通过 `RuntimeContext.stream_mode` 指定。
...
- `Pregel::invoke(input, runtime_context)` 已改为贴近源项目：内部调用 `stream(input, runtime_context)` 并消费 stream item。`StreamMode::Values` 返回最后一个 values payload；`StreamMode::Updates` 返回 `StateValue::List`，其中每个元素是 `{ step, mode, data }` 形状的 stream chunk。
- `Pregel::stream(input, runtime_context)` 已使用 `tokio::sync::mpsc` 返回管道 receiver，并由后台 task 建立 `PregelLoop`，随后立即调用 `enter()`，贴近源项目 `with SyncPregelLoop(...) as loop:` 的调用顺序。后台主循环现在可按 `tick -> execute -> after_tick` 推进到 `Done` 或递归限制错误；`StreamMode::Updates` 会在执行阶段发送 `{node: update}` 形状的最小更新 item，`StreamMode::Values` 会在 Update 阶段应用 writes 后发送 stream/output channel 快照。`execute` 和 `after_tick` 都是同步方法，`invoke` 通过 `stream` 复用同一运行路径；stream item 发送使用 `try_send`，receiver 关闭或管道暂满时会跳过该 stream item。`RuntimeContext.stream_mode` 可在单次运行中覆盖图规格默认 stream mode。`CompiledStateGraph` 通过 `Arc<Pregel>` 让后台 task 持有图规格，实际 loop 仍借用规格字段；每次运行复制 channels 和 managed，避免不同 stream 调用共享可变运行态。
- `tests/mvp_runtime.rs` 已作为公开 API 视角的 MVP 集成测试，覆盖 `invoke`、`invoke(Updates)` chunk 收集、默认 `stream`、多步 `stream(Values)`、`stream(Updates)`、`RuntimeContext.stream_mode` 覆盖、节点读取 `RuntimeContext.context`、多 state/output channel、`add_sequence` 顺序执行、条件入口、条件边、waiting edge、多次 invoke/stream 隔离、构图校验错误、空输入错误、unsupported command、节点错误包装和非法 state update。
- 已为 `RuntimeContext` 用户上下文字段和 `GraphError` 展示文本补充基础单元测试。

## 当前未完成

- `invoke` 已实现，但仅覆盖 fresh input 的同步 MVP 路径；尚未接入 checkpoint、resume、interrupt、运行配置或 typed output codec。
- `PregelLoop::tick`、`execute` 和 `after_tick` 已接入最小 Plan/Execution/Update 闭环，因此 `invoke()` 和 `stream()` 都能端到端推进图执行。`stream()` 仍一次只支持一个 `StreamMode`，不支持源项目的多 mode 列表输出形状；`invoke(Updates)` 的 chunk 列表用 `StateValue::List` 表达，而不是引入单独的 `InvokeResult` 类型。
...
- 条件边和 waiting edge 已能编译为 writer / trigger / barrier channel，并已有运行时集成测试覆盖实际调度执行；checkpoint、interrupt/resume 尚未接入运行时。
