...
- `tests/mvp_runtime.rs` 已作为公开 API 视角的 MVP 集成测试，覆盖 `invoke`、`invoke(Updates)` chunk 收集、默认 `stream`、`stream(Updates)`、`RuntimeContext.stream_mode` 覆盖、节点读取 `RuntimeContext.context`、条件边、waiting edge、多次运行隔离、空输入错误和 unsupported command。
