...
- `CompiledStateGraph` 已能由 `StateGraph::compile()` 生成，并持有可校验的 `Pregel` 容器；当前已提供 `invoke`、默认 `stream` 和 `stream_with_mode` 转发到内部 `Pregel`。
...
- `Pregel::invoke(input)` 已实现同步 MVP 主线：创建本次运行独立 `PregelLoop`，按 `enter -> tick -> execute -> after_tick` 循环推进到 `Done`，再通过 output channels 读取最终输出。单 output channel 返回该 channel 值；多个 output channels 返回 `StateValue::Object`，并跳过不可用 channel。
- `Pregel::stream(input)` 已使用 `tokio::sync::mpsc` 返回管道 receiver，并由后台 task 建立 `PregelLoop`，随后立即调用 `enter()`，贴近源项目 `with SyncPregelLoop(...) as loop:` 的调用顺序。后台主循环现在可按 `tick -> execute -> after_tick` 推进到 `Done` 或递归限制错误；`StreamMode::Updates` 会在执行阶段发送 `{node: update}` 形状的最小更新 item，`StreamMode::Values` 会在 Update 阶段应用 writes 后发送 stream/output channel 快照。`execute` 和 `after_tick` 都是同步方法，`invoke` 与 `stream` 复用同一组执行入口；stream item 发送使用 `try_send`，receiver 关闭或管道暂满时会跳过该 stream item。`Pregel::stream_with_mode(input, mode)` 可在单次调用中选择 `Values` 或 `Updates`，默认 `stream()` 仍使用图规格中的 `Values`。`CompiledStateGraph` 通过 `Arc<Pregel>` 让后台 task 持有图规格，实际 loop 仍借用规格字段；每次运行复制 channels 和 managed，避免不同 stream 调用共享可变运行态。
- `tests/mvp_runtime.rs` 已作为公开 API 视角的 MVP 集成测试，覆盖 `invoke`、默认 `stream`、`stream_with_mode(Updates)`、条件边、waiting edge、多次运行隔离、空输入错误和 unsupported command。
