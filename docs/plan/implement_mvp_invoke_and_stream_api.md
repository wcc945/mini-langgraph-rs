...
把当前“运行时主线已可跑”的状态收口成可验证 MVP：用户通过 `StateGraph` 构图、`compile()` 编译后，可以调用 `CompiledStateGraph::invoke(input, runtime_context)` 获取最终输出，也可以调用 `stream(input, runtime_context)` / `stream_with_mode(input, runtime_context, mode)` 接收 `values` 或 `updates` 输出。`RuntimeContext` 承载用户 context，并可通过可选 `stream_mode` 覆盖本次运行的 stream mode。
...
3. 增加按调用选择 stream mode 的入口。
   - 验证：默认 `stream()` 仍使用 `Values`；`RuntimeContext::with_stream_mode(StreamMode::Updates)` 和 `stream_with_mode(..., StreamMode::Updates)` 返回 updates item。
...
5. 补端到端运行测试。
   - 验证：`StateGraph -> compile -> invoke` 返回预期状态；`stream_with_mode(Updates)` 能收到节点更新；节点能读取 `RuntimeContext.context`；条件边和 waiting edge 编译后可实际调度执行。
6. 同步文档。
...
- 已实现 `Pregel::invoke(input, runtime_context)`，同步执行完整 Pregel loop 并返回最终输出。
- 已实现 `Pregel::stream(input, runtime_context)` 和 `Pregel::stream_with_mode(input, runtime_context, mode)`，单次调用可通过 `RuntimeContext.stream_mode` 覆盖默认 stream mode。
- 已实现 `CompiledStateGraph::invoke(input, runtime_context)`、`stream(input, runtime_context)`、`stream_with_mode(input, runtime_context, mode)` 转发。
